...
SkewHeap.LINK_COLOR = "#000000";
SkewHeap.HIGHLIGHT_CIRCLE_COLOR = "#000000";
SkewHeap.FOREGROUND_COLOR = "#000000";
...
SkewHeap.prototype.insertCallback = function (event) {
    var insertedValue;
    var val=4;
    if(String(this.insertField.value).length==1){
        val=1;
    }else if(String(this.insertField.value).length==2){
        val=2;
    }else if(String(this.insertField.value).length==3){
        val=3;
    }else{
        val=4
    }
    insertedValue = this.normalizeNumber(this.insertField.value, val);
